...
\section{Optional Public Website}
...
\section{GitHub Actions Deployment}

The repository includes a workflow file at \texttt{.github/workflows/pages.yml} to deploy the static site to GitHub Pages.

To activate the deployment:

\begin{enumerate}[leftmargin=1.5em]
  \item push the repository to GitHub,
  \item ensure the default branch is \texttt{main},
  \item open \texttt{Settings -> Pages},
  \item set the source to \textbf{GitHub Actions},
  \item rerun the workflow or push a new commit.
\end{enumerate}

This optional step broadens the reach of the project by making it easy to share publicly through a simple URL.

\section{Why This Submission Fits the Assignment}

\begin{itemize}[leftmargin=1.5em]
  \item The implementations are written in \textbf{Bash}, which satisfies the scripted-language requirement.
  \item Both demos include \textbf{simple graphics} in the terminal.
  \item The code explicitly labels the sections that demonstrate \textbf{recursion} and \textbf{iteration}.
  \item The repository optionally supports a \textbf{public GitHub Pages site}.
\end{itemize}
